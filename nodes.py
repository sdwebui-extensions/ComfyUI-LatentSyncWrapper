...
class LatentSyncNode:
    ...
    def inference(self, images, audio, seed, lips_expression=1.5, inference_steps=20):
        ...
        comfyui_temp = "/code/latentsync/temp"
        if os.path.exists(comfyui_temp):
            backup_name = f"{comfyui_temp}_backup_{uuid.uuid4().hex[:8]}"
            try:
                os.rename(comfyui_temp, backup_name)
        ...
        try:
            ...
            if not os.path.exists(ckpt_path) and os.path.exists(os.path.join(folder_paths.cache_dir, "models/LatentSync")):
                ckpt_path = os.path.join(folder_paths.cache_dir, "models/LatentSync/latentsync_unet.pt")
                whisper_ckpt_path = os.path.join(folder_paths.cache_dir, "models/LatentSync/whisper/tiny.pt")
